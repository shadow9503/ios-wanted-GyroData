...
extension ListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let replayViewController = ReplayViewController() // 셀루르면 푸시
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let actions2 = UIContextualAction(style: .normal, title: "Play", handler: { action, view, completionHaldler in
            ...
            secondView.xValue = DataManager.shared.saveList[indexPath.row].xData as! [Float]
            secondView.yValue = DataManager.shared.saveList[indexPath.row].yData as! [Float]
            secondView.zValue = DataManager.shared.saveList[indexPath.row].zData as! [Float]
            self.navigationController?.pushViewController(secondView, animated: true)
        })
